...
export const handler = async (event: FollowerStatusRequest): Promise<FollowerStatusResponse> => {
    const isFollowing = await new UserService().getIsFollowerStatus(event.authToken, event.user, event.selectedUser);
    return new FollowerStatusResponse(isFollowing, "", true);
};